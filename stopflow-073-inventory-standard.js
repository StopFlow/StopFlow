/* StopFlow 0.7.3 — standard générique d'inventaire fournisseur et page de fin. */
(function(){
  ...
  function supplierRecord(name){
    try{
      ...
      const list=Array.isArray(db?.suppliers)?db.suppliers:[];
      return list.find(item=>String(item?.name||'').trim().toLowerCase()===String(name||'').trim().toLowerCase())||null;
    }catch{return null}
  }
  ...
  function injectStyles(){
    ...
    style.textContent=`
      ...
      #sf73InventoryComplete .sf73-complete-hero{
        margin:6px 0 12px;
        padding:22px 18px;
        ...
        background:#f2fbf6;
        text-align:center;
        box-shadow:0 6px 18px rgba(13,35,62,.05)
      }
      #sf73InventoryComplete .sf73-complete-icon{
        width:58px;height:58px;margin:0 auto 10px;border-radius:50%;
        display:grid;place-items:center;background:#dff5e9;color:#118354;
      ...
      #sf73InventoryComplete .sf73-complete-meta{
        display:grid;grid-template-columns:1fr 1fr;gap:8px;margin:0 0 12px
      }
      #sf73InventoryComplete .sf73-complete-kpi{
        min-width:0;padding:11px 9px;border:1px solid var(--line);border-radius:12px;background:#fff;text-align:center
      }
      ...
      #sf73InventoryComplete .sf73-complete-card{
        padding:14px;border:1px solid var(--line);border-radius:14px;background:#fff;margin-bottom:10px
      }
  ...
  function ensureCompletionPage(){
    let page=document.getElementById('sf73InventoryComplete');
    if(page)return page;
    const main=document.querySelector('#app main,.main');
    if(!main)return null;
    page=document.createElement('section');
    page.id='sf73InventoryComplete';
  ...
  function revokePdf(){
    if(state.pdfUrl){
      try{URL.revokeObjectURL(state.pdfUrl)}catch{}
    }
    state.pdfBlob=null;
    state.pdfUrl=null;
    state.pdfFileName='';
  ...
  function completionStats(rows){
    ...
    return {
      ...
      orderQty:orderRows.reduce((sum,row)=>sum+Number(row.order||0),0)
    };
  }
  ...
  function completionExtraButtons(context){
    return [...state.extraActions.values()]
      .filter(action=>{
        try{return typeof action.visible==='function'?action.visible(context)!==false:action.visible!==false}catch{return false}
      })
  ...
  async function sharePdf(){
    ...
    const payload={
      title:`StopFlow — ${state.savedOrder.supplier}`,
      text:`Inventaire validé ${state.savedOrder.number||''} — ${state.savedOrder.supplier}`,
      files:[file]
    };
  ...
  function prepareEmail(){
    ...
    const body=[
      'Bonjour,',
      '',
      `Veuillez trouver la commande issue de l’inventaire StopFlow ${order.number||''}.`,
      `Fournisseur : ${order.supplier}`,
      `Références à commander : ${stats.orderLines}`,
      `Quantité totale : ${stats.orderQty}`,
      '',
      'Le PDF généré par StopFlow peut être joint à ce message.',
      '',
      'Cordialement'
    ].join('\n');
  ...
  function newInventory(){
    ...
    if(zone==='salle'&&typeof window.stopflow073SalleInventoryFlow?.open==='function'){
      window.stopflow073SalleInventoryFlow.open();
      return;
    }
    if(zone&&zone!=='home'&&typeof window.stopflow070CardNavigation?.openZone==='function'){
      window.stopflow070CardNavigation.openZone(zone);
      return;
    }
  ...
  function bindCompletionActions(pageNode,context){
    ...
    bindTap(pageNode.querySelector('#sf73DoneNew'),newInventory);
      ...
      <div class="sf73-complete-shell">
        ...
        <div class="sf73-complete-card">
          ...
          ${!pending?`<p class="sf73-complete-email">${supplierEmail
            ...
            :'Aucun e-mail fournisseur n’est encore enregistré. Le bouton « Préparer un e-mail » ouvrira un nouveau message sans destinataire.'}</p>`:''}
        </div>
      </div>
    `;
  ...
  async function finalizeValidation(){
    ...
    try{
      ...
      const message=typeof cloudErrorMessage==='function'
        ?cloudErrorMessage(error,'Validation impossible.')
        :(error?.message||'Validation impossible.');
      alert(message);
    }finally{
      state.saving=false;
  ...
  function installValidationOwner(){
    ...
    document.addEventListener('pointerup',event=>{
      ...
      event.stopPropagation();
      event.stopImmediatePropagation();
      const tap=!gesture.moved&&button===gesture.button&&Date.now()-gesture.startedAt<1000;
      if(tap)finalizeValidation();
    },true);
  ...
  function registerCompletionAction(action){
    ...
    state.extraActions.set(String(action.id),{
      id:String(action.id),

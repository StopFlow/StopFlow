...
(function(){
  ...
  function injectStyles(){
    ...
    style.textContent=`
      @media(max-width:950px){
        ...
        #inventory #zeroAll{min-height:44px!important;touch-action:manipulation!important;-webkit-tap-highlight-color:transparent!important}
        ...
        #inventory #inventoryRows .qty button{width:48px!important;height:46px!important;padding:0!important;font-size:23px!important;line-height:1!important;touch-action:manipulation!important;-webkit-tap-highlight-color:transparent!important}
  ...
  function enhanceRows(){
    ...
    const zero=document.getElementById('zeroAll');
    if(zero)zero.setAttribute('aria-label','Mettre tous les stocks à zéro');
  }

  function inventoryAction(target){
    const control=target?.closest?.('[data-minus],[data-plus],#zeroAll')||null;
  ...
  function installMobileControls(){
    ...
    document.addEventListener('click',event=>{
      if(!isMobile())return;
      const control=event.target?.closest?.('[data-minus],[data-plus],#zeroAll')||null;
      if(!control)return;

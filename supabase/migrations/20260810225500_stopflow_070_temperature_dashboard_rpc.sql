...
begin
  ...
  if v_can_read then
    ...
    into v_rounds
    from (select * from public.temperature_rounds where department='cuisine' order by completed_at desc limit 20) r;
    select coalesce(jsonb_agg(to_jsonb(t) order by t.recorded_at desc), '[]'::jsonb)
    into v_readings
    from (select * from public.temperature_readings where department='cuisine' order by recorded_at desc limit 160) t;
  end if;
  return jsonb_build_object('equipment',v_equipment,'rounds',v_rounds,'readings',v_readings,'can_read',v_can_read,'can_manage',v_can_manage);
end;
$$;
